...
std::vector<mandeye::PointRGB> mandeye::load(const std::string &lazFile) {
	...
	while (p_count < npoints) {
		...
		p.rgb[0] = point->rgb[0] / 255.0f;
		p.rgb[1] = point->rgb[1] / 255.0f;
		p.rgb[2] = point->rgb[2] / 255.0f;
		p.rgb[3] = point->rgb[3] / 255.0f;
